...
public:
	...
	bool insert(Node);  //insert a node 
	bool remove(Node x);  //remove a node
	bool remove(int index);  //remove a certain index node
	bool sort();   //sort binary heap using merge sort
	...
	void merge(Node *a, Node *aux, int lo, int mid, int hi);  //merge sort
