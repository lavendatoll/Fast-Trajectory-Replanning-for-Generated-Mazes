...
bool BinaryHeap::sort()
{
	...
	else
	{
		...
		{
			if ((cnt - 1)/2 >= 0 && bHeap[cnt].get_f() <= bHeap[(cnt - 1) / 2].get_f())
			{
				Node temp = bHeap[cnt];
...
void BinaryHeap::merge(Node * a, Node * aux, int lo, int mid, int hi)
{
	for (int cnt = 0; cnt <size; cnt++)
		aux[cnt] = a[cnt];
	int i = lo, j = mid + 1;
	for (int k = lo; k <= hi; k++)
	{
		if (i > mid)
		    ...
		    a[k] = aux[i++];
		else if ((aux[j].get_f()) < (aux[i].get_f())) 
			a[k] = aux[j++];
		else 
			a[k] = aux[i++];
	}
